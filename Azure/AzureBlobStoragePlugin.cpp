...
IStorage* AzureBlobStoragePluginFactory::CreateStorage(const std::string& nameForLogs, const OrthancPlugins::OrthancConfiguration& orthancConfig)
{
  ...
  if (orthancConfig.IsSection(GetConfigurationSectionName()))
  {
    ...
    {
      spdlog::error("AzureBlobStorage/ConnectionString configuration missing.  Unable to initialize plugin");
      return nullptr;
    }

    if (!pluginSection.LookupStringValue(containerName, "ContainerName"))
    {
      spdlog::error("AzureBlobStorage/ContainerName configuration missing.  Unable to initialize plugin");
      return nullptr;
    }
  ...
  {
    spdlog::warn("AzureBlobStorage: you're using an old configuration format for the plugin.");
    ...
    if (!pluginSection.LookupStringValue(containerName, "ContainerName"))
    {
      spdlog::error("BlobStorage/AccountName configuration missing.  Unable to initialize plugin");
      return nullptr;
    }
    ...
    {
      spdlog::error("BlobStorage/ContainerName configuration missing.  Unable to initialize plugin");
      return nullptr;
    }
  ...
  try
  {
        ...
        && createContainerIfNotExists)
    {
      ...
      if (containerCreated)
      {
        spdlog::warn("Blob Storage Area container has been created.  **** check in the Azure console that your container is private ****");
      }
    }

    spdlog::info("Blob storage initialized");
  ...
  {
    spdlog::error(std::string("AzureBlobStorage plugin: failed to initialize plugin: ") + e.what());
    return nullptr;
  }
...
//
//}
AzureBlobStoragePlugin::AzureBlobStoragePlugin(const std::string& nameForLogs,
                                               const as::cloud_blob_client& blobClient,
                                               const as::cloud_blob_container& blobContainer,
                                               bool enableLegacyStorageStructure,
                                               bool storageContainsUnknownFiles)
          ...
          storageContainsUnknownFiles_(storageContainsUnknownFiles)
{
    ...
    logger->sinks()[1]->set_formatter(std::make_unique<spdlog::pattern_formatter>("[%Y-%m-%d %H:%M:%S.%e] %v", spdlog::pattern_time_type::local, "json"));

    // Set the global logger
    spdlog::set_default_logger(logger);

    // Log an example message
    spdlog::info("AzureBlobStoragePlugin initialized");
}
